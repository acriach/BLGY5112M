...
Cohort A because the p values do not have to be as small to be below 0.05 and to reject the null hypothesis.
...
There are various packages which do power analysis in R. [pwr](https://cran.r-project.org/web/packages/pwr/index.html) does simpler analyses up to `lm` and [simR](https://cran.r-project.org/web/packages/simr/index.html) does more complex mixed models and glms.
...
Power improves to around 20%. Instead of manually plugging in different values for n, we could make R run the power analysis for many different sample sizes. This code calculates and plots power for samples sizes from 2 to 200.
```{r}
...
Effect sizes should be reported in results. If a effect size is not given, it can sometimes be calculates. For example, d can be calculated if the means and standard deviations are given.  
::::
