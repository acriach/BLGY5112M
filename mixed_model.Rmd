...
If you have learned how to run models using lm, write out how you would interpret the output from the summary.
...
Notice it is smaller than the `Std. Error` for `bodyLength`. That means that the effect (think of effect as the slope of the fitted line in a scatterplot) is no different to 0. In other words, there is no effect.
...
Do an internet search to find the function and the code for a binominal mixed model. Adapt the code imagining that your response variable is `passFail`, the fixed effect is `bodyLength` and random effect is `mountainRange`. 

<details>
  <summary>**Answer**</summary>
```{r, eval = FALSE}
glmer(passFail ~ bodyLength + (1|mountainRange), data = dragons, family = binomial)
```
...
Do an internet search to find an example of a study that includes a nested design. You have 3 tasks:  
...
Tip - searching for a tutorial or similar on a crossed designed mixed model will provide both an example and the code.
